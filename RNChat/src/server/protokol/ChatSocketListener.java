...
/**
 * Wenn gestartet, horcht er auf einem bestimmten Port und öffnet so viele Verbindungen wie es die Konfiguration
...
public class ChatSocketListener extends Thread{
	...
	//wird benötigt um den server "sanft" zu beenden, also keine neuen verbindungen aufbauen zu lassen..
	 ...
	 * @param clients maximale Anzahl an Clients (abhängig vom Modus) (default ist 10)
	 * @param modus ist für die Erstellung der Verbindungsthreads mit den Clients. {@code ServerExecutor.FixedThreadPool} (default) erzeugt einen Pool an Threads mit fester Anzahl. {@code ServerExecutor.WorkStealingPool} sorgt für maximale parallelität. Wird von Java intern verwaltet..  
	 */
	...
	@Override
	public void run() {
		//sonst würde es die möglichkeit geben das der server angehalten wird bevor er gestartet wird..
		listen=true;
...
//			executorPool = Executors.newFixedThreadPool(maxClients);
		...
		while(listen){
			...
			try {
				...
				//es werden so lange verbindungen angenommen bis der pool erschöpft ist
	...
	/**
	 * Sorgt dafür das der Server keine weiteren Verbindungen mehr annimmt und versucht den ServerSocket zu Schliessen 
	 */
